...
#model1gender = sar_probit_mcmc(y = y, X = X, W = W, ndraw=1000, burn.in=100, showProgress=TRUE) # eigen versie met z zelf gemaakt
...
write.table(imputed, "imputed_cutmodel_gender75random.csv", sep = ";", quote = F, col.names=F, row.names=F)
...
write(paste(impmodel.10$beta, impmodel.10$beta_std, impmodel.10$rho, mean(error.estimates.10$error), sd(error.estimates.10$error)), "cutmodel10Genderrandom.txt")
write(paste(impmodel.25$beta, impmodel.25$beta_std, impmodel.25$rho, mean(error.estimates.25$error), sd(error.estimates.25$error)), "cutmodel25Genderrandom.txt")
write(paste(impmodel.35$beta, impmodel.35$beta_std, impmodel.35$rho, mean(error.estimates.35$error), sd(error.estimates.35$error)), "cutmodel35Genderrandom.txt")
write(paste(impmodel.50$beta, impmodel.50$beta_std, impmodel.50$rho, mean(error.estimates.50$error), sd(error.estimates.50$error)), "cutmodel50Genderrandom.txt")
write(paste(impmodel.75$beta, impmodel.75$beta_std, impmodel.75$rho), "cutmodel75Genderrandom.txt")
...
model10 = sar_combined_mcmc_imp_fbs(y = yf10, x = X, W = W, ndraw = 25000, burn.in = 1000, method = c("probit"),
                                   ...
                                   model = "random10_lflag0_repl2")

model25 = sar_combined_mcmc_imp_fbs(y = yf25, x = X, W = W, ndraw = 25000, burn.in = 1000, method = c("probit"),
                                   prior=list(a1=1, a2=1, beta=rep(0, ncol(X)), T=diag(ncol(X))*1e12, lflag = 2),
                                   m=10, thinning=1, computeMarginalEffects=TRUE, showProgress=TRUE, start = start,
                                   model = "random25_lflag2")
...
write.table(imputed75, "imputed_cutmodel_gender75random.csv", sep = ";", quote = F, col.names=F, row.names=F)
...
setwd("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/")
setwd("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/bdraws_random/")
...
lines(density(bdraws50cut$V2), col = "red3")
...
lines(density(bdraws50fbs$V2), col = "royalblue2")
...
effective0 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/rhoestimaterandom/rhoestimate_nomissing_cut.txt")

# read in the rho estimates with missing data
effective10cut = read.table("rhoestimaterandom/rhoestimaterandom10cut.txt",  xlab = "draws", ylab = "rho")
effective10fbs = read.table("rhoestimaterandom/rhoestimaterandom10fbs.txt", xlab = "draws", ylab = "rho")

effective25cut = read.table("rhoestimaterandom/rhoestimaterandom25cut.txt",  xlab = "draws", ylab = "rho")
effective25fbs = read.table("rhoestimaterandom/rhoestimaterandom25fbs.txt",  xlab = "draws", ylab = "rho")

effective35cut = read.table("rhoestimaterandom/rhoestimaterandom35cut.txt",  xlab = "draws", ylab = "rho")
effective35fbs = read.table("rhoestimaterandom/rhoestimaterandom35fbs.txt",  xlab = "draws", ylab = "rho")

effective50cut = read.table("rhoestimaterandom/rhoestimaterandom50cut.txt",  xlab = "draws", ylab = "rho")
effective50fbs = read.table("rhoestimaterandom/rhoestimaterandom50fbs.txt",  xlab = "draws", ylab = "rho")

effective75cut = read.table("D:/Documents/downloads/ToreOpsahl/rhoestimaterandom75cut.txt")
#effective75cut = read.table("rhoestimaterandom/rhoestimaterandom75cut.txt", xlab = "draws", ylab = "rho")
effective75fbs = read.table("rhoestimaterandom/rhoestimaterandom75fbs.txt", xlab = "draws", ylab = "rho")
...
# cut 
imputed10 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender10.csv", sep = ";")
imputed25 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender25.csv", sep = ";")
imputed35 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender35.csv", sep = ";")
imputed50 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender50.csv", sep = ";")
imputed75 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender75.csv", sep = ";")
# first process this output

# fbs
imputed10 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender10.csv", sep = ";")
imputed25 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender25.csv", sep = ";")
imputed35 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender35.csv", sep = ";")
imputed50 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender50.csv", sep = ";")
imputed75 = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender75.csv", sep = ";")
...
combined.y = as.data.frame(cbind(y[unique(missing.sample.10)], mean.z$sign))
combined.y = as.data.frame(cbind(y[unique(missing.sample.25)], mean.z$sign))
combined.y = as.data.frame(cbind(y[unique(missing.sample.50)], mean.z$sign))
combined.y = as.data.frame(cbind(y[unique(missing.sample.75)], mean.z$sign))

combined.y$diff = combined.y$V1 - combined.y$V2
sum(abs(combined.y$diff))

#  cut
77 / length(unique(missing.sample.10))
194 / length(unique(missing.sample.25))
279 / length(unique(missing.sample.35))
399 / length(unique(missing.sample.50))
667 / length(unique(missing.sample.75))

#  cut
77 / length(unique(missing.sample.10))
190 / length(unique(missing.sample.25))
276 / length(unique(missing.sample.35))
398 / length(unique(missing.sample.50))
615 / length(unique(missing.sample.75))

# zijn het dezelfde mensen
imputed10a = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_cutmodel_gender10.csv", sep = ";")
imputed10b = read.table("D:/Documents/downloads/ToreOpsahl/bayes_random_gender_28052018/imputed_random/imputed_fbsmodel_gender10.csv", sep = ";")

# two ways of doing this, take the mean \hat{z} or take the mode indicator function of \hat{z}
mean.z = as.data.frame(as.matrix(colMeans(imputed10a)))
mean.z$sign <- NA
mean.z[mean.z$V1 < 0,]$sign <- 0
mean.z[mean.z$V1 > 0,]$sign <- 1
table(mean.z$sign)
...
combined = as.data.frame(cbind(y[unique(missing.sample.cluster)], mean.z))
colnames(combined) = c("y", "meanz", "signz")
combined$diff = combined$y - combined$signz
sum(combined$diff)/nrow(combined)

####
write.table(impmodel.75$bdraw[[1]], "bdraws.snow.cut75.flag2", sep = ";", quote = F, col.names = F, row.names = F)

write.table(model75$bdraw[[1]], "bdraws.snow.fbs75.flag1", sep = ";", quote = F, col.names = F, row.names = F)

# calculate brier score for every person
f = 1 # forecast
o = 1 # observed
1/n*(f-o)^2 # outcome = 0, best achievable

f = 1
o = 0
1/n*(f-o)^2 # outcome = 1, worst achievable
...
z.indicator = ifelse(impmodel.10$y_imp[[1]] > 0, 1, 0)
z.perc = colSums(z.indicator) / nrow(z.indicator)
z.table = cbind(z.perc, y[missing.sample.10])
colnames(z.table) = c("f", "o")
write.table(brier.score(as.data.frame(z.table)), "brier.score.10random_flag0.txt", quote = F, sep = ";", row.names = F, col.names=F)
...
z.indicator = ifelse(model75$y_imp[[1]] > 0, 1, 0)
z.perc = colSums(z.indicator) / nrow(z.indicator)
z.table = cbind(z.perc, y[missing.sample.75])
colnames(z.table) = c("f", "o")
write.table(brier.score(as.data.frame(z.table)), "brier.score.fbs.75random_flag2.txt", quote = F, sep = ";", row.names = F, col.names=F)
