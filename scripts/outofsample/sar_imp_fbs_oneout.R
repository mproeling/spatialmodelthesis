  ...
  if(prior2$novi == 0){ # fit heteroscedastic model 
    ...
    while(iter <= ndraw*thinning){ # start sampling;
      for(i in 1:Yk){
        ...
        if(iter == 1 - burn.in){
          write.table(rho[i], paste0("rhoestimate_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
          write.table(t(bhat), paste0("beta_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
          write.table(imp, paste0("imp_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
        } else {
          write.table(rho[i], paste0("rhoestimate_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
          write.table(t(bhat), paste0("beta_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
          write.table(imp, paste0("imp_", model, "fbs.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
        }
