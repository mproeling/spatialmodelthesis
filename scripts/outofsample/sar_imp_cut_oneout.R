...
sar_combined_mcmc_imp_cut = function(y, x, W, ndraw=1000, burn.in=100, thinning=1, 
  ...
  #y = yf_match;  x = X;  method = "probit";  ndraw=1000 ;  burn.in=100;  thinning=1;  prior=list(a1=1, a2=1, beta=rep(0, ncol(X)), T=diag(ncol(X))*1e12, lflag = 0);
  #rho = 0.39;  rhosd = 0.16;  m=10;  computeMarginalEffects=TRUE;  showProgress=TRUE;
  
  #y = yf1;  x = X;  method = "probit";  ndraw=1000 ;  burn.in=100;  thinning=1;  prior=list(a1=1, a2=1, beta=rep(0, ncol(X)), T=diag(ncol(X))*1e12, lflag = 0);
  #rho = 0.39;  rhosd = 0.16;  m=10;  computeMarginalEffects=TRUE;  showProgress=TRUE; model = missing_person_id;
  ...
  if(prior2$novi == 0){ # fit heteroscedastic model 
    ...
    while(iter <= ndraw*thinning){ # start sampling;
      for(i in 1:Yk){
        ...
        if(iter == 1 - burn.in){
          write.table(rho[i], paste0("rhoestimate_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
          write.table(t(bhat), paste0("beta_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
          write.table(imp, paste0("imp_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
        } else {
          write.table(rho[i], paste0("rhoestimate_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
          write.table(t(bhat), paste0("beta_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
          write.table(imp, paste0("imp_", model, "cut.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
        }
