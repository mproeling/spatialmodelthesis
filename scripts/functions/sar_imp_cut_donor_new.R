...
sar_combined_mcmc_imp_cut_cluster = function(y, x, W, ndraw=1000, burn.in=100, thinning=1, 
                                     ...
                                     method=vector("character", meth.length = ncol(x)), model=as.numeric(1), Nd = 5){
  ...
  #y = yf.cluster;  x = X;  method = "probit";  ndraw=100 ;  burn.in=25;  thinning=1;  prior=list(a1=1, a2=1, beta=rep(0, ncol(X)), T=diag(ncol(X))*1e12, lflag = 0);
  #rho = 0.39;  rhosd = 0.16;  m=10;  computeMarginalEffects=TRUE;  showProgress=TRUE; model = "cluster10"
    ...
    while(iter <= ndraw*thinning){ # start sampling;
      for(i in 1:Yk){
        if(i == 1){
          ...
          Woo = W[obs[[i]], obs[[i]]] # for the imputation
        ...
        # x_{obs}
        Iobs = sparseMatrix(i=1:Nobs[[i]], j=1:Nobs[[i]], x=1) # sparse identity matrix
        element1.obs = qr(Iobs - rho[i]*Woo)
        element2.obs = (x_obs%*%bhat) + (rho[i] * Woo%*%(y[obs[[i]], i]))
        # should be N(0, Imiss*sige[i]) but Imiss is always 1 
        imp.error.obs = rnorm(Nobs[[i]], 0, sige[i])
        element3.obs = solve(element1.obs, matrix(imp.error.obs))
        
        # hence:
        imp = cbind(missing[[i]], as.data.frame(as.matrix(solve(element1, element2) + element3)))
        imp.obs = cbind(obs[[i]], as.data.frame(as.matrix(solve(element1.obs, element2.obs) + element3.obs)))
        colnames(imp) = c("id", "y_pred")
        colnames(imp.obs) = c("id", "y_pred")
        y_pred = rbind(imp, imp.obs)
        y_pred = y_pred[order(y_pred$id),]
        
        ## y_pred contains everybody in the dataset and contains \hat(y) for all individuals, this is required to get the donors
        # dim(y_pred)
        ...
        if(iter > 0){
          ...
          y_imp[[i]][iter,] <- y[missing[[i]],i] # put values in matrix to get posterior conditional distribution of imp values
          if(priors$mm != 0){
            rsave[iter,,i] = rval[i]
          }
        }
    ...
    while(iter <= ndraw*thinning){ # start sampling;
      for(i in 1:Yk){
        ...
        if(method[i] == "probit"){
          ...
          # the original function is too instable and resulted in too much NaN values 
          ...
          if(iter <= 0){
            write.table(t(imp.obs$y_pred), "X_obs_predictors.csv", sep = ";", quote = F, col.names = F, row.names = F)
          } else {
            write.table(t(imp.obs$y_pred), "X_obs_predictors.csv", sep = ";", quote = F, col.names = F, row.names = F, append = TRUE)
          }
          ...
          if(iter > 0){
            ...
            for(MIS in 1:Nmiss[[i]]){
              ...
              y[missing[[i]][MIS], i] = mean(donor_values)
              ...
              if(iter == 1 - burn.in){
                write.table(output, paste0("clusteroutput_donors.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
              } else {
                write.table(output, paste0("clusteroutput_donors.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
              }
            }
          }
          options(warn=0)
