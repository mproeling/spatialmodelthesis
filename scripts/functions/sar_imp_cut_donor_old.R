        ...
        if(method[i] == "probit"){
          ...
          if(iter > 0){
            ...
            for(MIS in 1:Nmiss[[i]]){
              ...
              if(iter == 1 - burn.in){
                write.table(output, paste0("clusteroutput.txt"), col.names = F, row.names = F, quote = F, sep = "\t")
              } else {
                write.table(output, paste0("clusteroutput.txt"), col.names = F, row.names = F, quote = F, sep = "\t", append = T)
              }
            }
          }
          options(warn=0)
