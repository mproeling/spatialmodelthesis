# defintion of phi currently only allows to include 1 ordered probit
...
sar_combined_mcmc_imp_fbs = function(y, x, W, ndraw=1000, burn.in=100, thinning=1, 
                            ...
                            multi.method = c("sequential", "multivariate")){

  y = yf1

  x = as.matrix(X);  ndraw=100000;  burn.in=1000;  thinning=1;  prior=list(a1=1, a2=1, beta=rep(0, ncol(X)), T=diag(ncol(X))*1e12, lflag = 0);  rho = 0.39;
  rhosd = 0.16;  m=10;  computeMarginalEffects=TRUE;  showProgress=TRUE;  method = c("orderedprobit", "continuous"); multi.method = "multivariate";
  ...
  for(i in 1:Yk){
    ...
    if(method[i] == "probit"){
      ...
      phi=c(-Inf, 0:(max(y[[i]], na.rm = T)-1), Inf)
  ...
  if(prior2$novi == 0){ # fit heteroscedastic model 
    ...
    while(iter <= ndraw*thinning){ # start sampling;
      for(i in 1:Yk){
        ...
        # impute 
        Wy = as.data.frame(get(paste0("Wy", i))[missing[[i]],])
        imp = rnorm(Nmiss[[i]], (rho[[i]] * Wy + x_mis%*%bhat)[[1]], sd(get(paste0("y", i)))) # Nmiss = number of draws, Mean, Sd
        ...
        if(iter > burn.in){ # if we are past burn-in, save the draws
          bsave[[i]][iter-burn.in,] = t(bhat) # bsave[[i]] = Yi, order of coefficients is {Yk, Yk-1, ..., Y1} 
          ssave[iter-burn.in,,i] = sige[i]
          psave[iter-burn.in,,i] = rho[i]
          vmean[[i]] = vmean[[i]] + vi[[i]]
          y_imp[[i]][iter-burn.in,] <- imp # put values in matrix to get posterior conditional distribution of imp values
          if(priors$mm != 0){
            rsave[iter-burn.in,,i] = rval[i]
          }
        }
      ...
      for(i in 1:Yk){
        ...
        if(method[i] == "probit"){
          ...
          if (iter > 0) {
            ...
            psave[iter,,i]        = rho[i]
            vmean[[i]]            = vmean[[i]] + vi[[i]]
            y_imp[[i]][iter,]     = imp # put values in matrix to get posterior conditional distribution of imp values
          ...
          z=c()          
          for(QQ in 1:n){
            z[QQ] = rtruncnorm(1, a=lower[[QQ]], b=upper[[QQ]], mean = mu[[i]][QQ], sd = 1)
          }  
        ...
        # finished calculting the individual parts, start the multivariate imputation
        # this is terribly inefficient and currently only works with 2 covariates
        if(i == Yk){
          rho.matrix = diag(rho)
          x_obs.all = cbind(x_obs[,i], get(paste0("y", i)), x_obs[,-c(1:i)])
          colnames(x_obs.all)[c(1:i)] = paste0("Y", seq(1,i)) 
          x_obs.all.sigma = cov(x_obs.all)
          
          # get beta for Y1 including Y2
          x_obs.Y1 = cbind(1,x_obs.all[,-1])
          colnames(x_obs.Y1)[1] = "Intercept"
          xpx = t(x_obs.Y1)%*%x_obs.Y1
          xpxI <- solve(xpx)               # (X'X)^{-1}       # k x k
          xxpxI <- xpx %*% xpxI              # X(X'X)^(-1)     # n x k (better, compromise)

          # normally replace i with 1 but the dimensions are identical compared to i = 2; we still need 4 regression coeff (Y2, B1-Bk) + intercept
          assign(paste0("priors",i), sar_parse_multiple(prior, k, x_obs, i))
          TI = solve(get(paste0("priors",i))$diagT)
          TIc = TI %*% get(paste0("priors",i))$diffuseprior # redefine diffuse prior in sar_parse (priors in array after sarparse?)
          ...
          AI = qr.solve(xpx + sige[i]*TI, diag(k2))
          ys = get(paste0("y", 1)) - rho[i]*get(paste0("Wy", 1))
          b = t(as.matrix(x_obs.Y1)) %*% ys + sige[i]*TIc
          b0 = qr.solve(xpx + sige[i]*TI, b)
          bhat = norm_rnd(sige[i]*AI) + b0
          ...
          # covariates are fully observed so we dont need  take those values
          imputation = sort(unique(unlist(missing)))
          
          x_obs.all = cbind(1, x_obs.all)
          colnames(x_obs.all)[1] = "Intercept"
          x_mis1 = as.matrix(x_obs.all[missing[[1]],])
          x_mis1 = cbind(as.numeric(row.names(x_mis1)), x_mis1)
          x_mis2 = as.matrix(x_obs.all[missing[[2]],])
          x_mis2 = cbind(as.numeric(row.names(x_mis2)), x_mis2)
          x_mis.all = unique(rbind(x_mis1, x_mis2))
          x_mis.all = x_mis.all[order(x_mis.all[,1]),]
          
          # first model Y1 
          rhoWy1 = rho[[1]] * Wy1 
          x_mis.all.imputed = cbind(x_mis.all, rhoWy1[unique(x_mis.all[,1]),] + as.matrix(x_mis.all[,-c(1,3)]) %*% t(as.matrix(beta.all[[1]])))
          
          # second model Y2
          rhoWy2 = rho[[2]] * Wy2 
          x_mis.all.imputed = cbind(x_mis.all.imputed, rhoWy1[unique(x_mis.all[,1]),] + as.matrix(x_mis.all[,-c(1,4)]) %*% t(as.matrix(beta.all[[2]])))
          colnames(x_mis.all.imputed)[c(ncol(x_mis.all.imputed)-1, ncol(x_mis.all.imputed))] = c("Y1imp", "Y2imp")

          for(QQ in 1:length(imputation)){
            person = imputation[QQ]
            
            if(person %in% missing[[1]]){
              person.values = x_mis.all.imputed[x_mis.all.imputed[,1] == person, ] 
              mu.person = c(person.values[c(8, 4, 5:7)]) # Y1 predicted (XtB), Y2 observed, X1 - Xk
              im.person = mvrnorm(n = 1, mu = mu.person, Sigma = x_obs.all.sigma, tol = 1e-6)
              y[person, 1] <- im.person[1] # put values in y matrix for imputation of y2 and beyond, and update the estimated values of Wy (above)
            }
            
            if(person %in% missing[[2]]){
              person.values = x_mis.all.imputed[x_mis.all.imputed[,1] == person, ] 
              mu.person = c(person.values[c(9, 3, 5:7)]) # Y2 predicted (XtB), Y1 observed, X1 - Xk
              im.person = mvrnorm(n = 1, mu = mu.person, Sigma = x_obs.all.sigma, tol = 1e-6)
              y[person, 2] <- im.person[1] # put values in y matrix for imputation of y2 and beyond, and update the estimated values of Wy (above)
            }
          }
          
          if(iter > burn.in){ # if we are past burn-in, save the draws
            y_imp[[1]][iter-burn.in,] <- y[person, 1] # put values in matrix to get posterior conditional distribution of imp values
            y_imp[[1]][iter-burn.in,] <- y[person, 2] # put values in matrix to get posterior conditional distribution of imp values
          }
        }

        # finished imputation, next 
      }
      iter = iter + 1
      if(showProgress){setTxtProgressBar(pb, iter)}
    } # End the sampling
