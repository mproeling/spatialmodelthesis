rm(list = ls(all = TRUE))
################################################################################
# Dino Dittrich, adapted by mp roeling
# 24/04/2017
#
...
# metropolis hasting sampling of rho and beta1
mh.up.rhobeta1.N=function(mu,Sigma,lb,ub,y,Wy,EW,lndet,s2,Xbetatilde,g,SS,cval,Ay,m,std2) {
  
  # Sample one random number From The Truncated Multivariate Normal Distribution
  pval=c(rtmvnorm(1,mean=mu,sigma=Sigma,lower=c(lb,-Inf),upper=c(ub,Inf)))
  
  Ay_pval=y-pval[1]*Wy
  lndet_pval=sum(log(1-pval[1]*EW))
  # accepted yes / no; draw from Multivariate Normal Density and Random Deviates
   if(log(runif(1))<
    lndet_pval-lndet
...
MCMC.N=function(R=1e4,
                y, # vector of dependent variable
                X, # vector of intercept + independent variable(s)
                W, # adjacency matrix with network links
                m, # mean for rho
                std2, # std2 for rho
                startval){
 g=length(y)    # Number of persons
 Id=diag(g)     # identity matrix
 ones=rep(1,g)  # list of ones of size g
 
 #Wy=c(W%*%y)    # Wy, combination of network effects and y
 Wy=W%*%y    # mp added
 yWones=sum(Wy) 
 yWWy=sum(Wy**2)
 EW=Re(eigen(W)$values) # Eigenvalues of W, of length g
 tau2=sum(EW**2)        # sum of squared Eigenvalues 
 lb=1/min(EW)           # inverse of minimal value of EW
 ub=max(EW)             # max value of EW
 
 if(!is.matrix(X)){X = as.matrix(X)} # mp added
 k=ncol(X)              # number of covariates
 Xtilde=X[,-1]          # covariates without intercept
 XtXi=solve(t(X)%*%X)
 XtXiXt=XtXi%*%t(X)
 M=Id-X%*%XtXiXt        # Identity matrix-covariates*XtXiXt (M in paper, page 216)
 ...
 Sigmai22_help=g
  ...
  if(missing(startval)) {
    ...
    MWy=M%*%Wy
    yMWy=sum(y*MWy)
    yWMWy=sum(MWy**2) # flat prior, see page 216 of paper
    
    # finds the optimum by funding the minimum of function loglik
    ...
    betaS=c(XtXiXt%*%Ay)
    beta1S=as.numeric(betaS[[1]][1])
    betatildeS=as.numeric(betaS[[1]][-1])
    s2S=yAMAy/g
  }else{
    rhoS=startval[1]
 ...
 sigma2.draws=NULL
 rho.draws[1]=rhoS
 beta.draws[1,]=as.numeric(betaS[[1]])
 sigma2.draws[1]=s2S
  ...
  for (r in 2:R) {
   ...
   restilde=c(y-Xtilde%*%betatildeS) # residuals (without intercept)
...
# compare to normal lm without correcting for network structure
# regression coefficients
colMeans(out$beta.draws)
X1 = X[,2]; X3 = X[,3]; X3 = X[,4]
test = lm(y ~ X1 + X2 + X3)
summary(test)

# compare against sarprobit
# set network size (n), network density(d), and generate random W
set.seed(2)
n=50
d=.05
...
ycat = y
ycat[ycat >= -1.867546] <- 1
ycat[ycat < -1.867546] <- 0

X1 = X[,2]; X2 = X[,3]; X3 = X[,4]

fit3 <- sarprobit(ycat ~ X1 + X2 + X3, W=W, ndraw=10000, burn.in = 1000, showProgress=TRUE) 
summary(fit3)
