...
# 24/04/2017
#
# Program to sample from a posterior based on a flat prior for rho, beta, and ln(sigma2) for the NAM
# Input in main function (MCMC.F):
#   - # of iterations: R
...
mh.up.rhobeta1.F=function(mu,Sigma,lb,ub,y,Wy,EW,lndet,s2,Xbetatilde,g,SS,cval,Ay) {
...
MCMC.F=function(R=1e4,y,X,W,startval) {
 g=length(y)
 Id=diag(g)
 ones=rep(1,g)
 Wy=c(W%*%y)
 ...
 sigma2.draws[1]=s2S
  ...
  for (r in 2:R) {
   ...
   mu=c(mu1,mu2)
   Draw=mh.up.rhobeta1.F(mu,Sigma,lb,ub,y,Wy,EW,lndet,s2S,XbetatildeS,g,SS,c(rhoS,beta1S),Ay)   
   rhoS=Draw$value[1]
   beta1S=Draw$value[2]
   Ay=Draw$Ay
   lndet=Draw$lndet
...
#####
# trace plot of posterior draws for rho with data-generating value added
plot(out$rho.draws,type="l",ylab=expression(rho))
abline(h=rho,col=2)
# empirical autocorrelation function for posterior draws for rho
acf(out$rho.draws)
# marginal posterior density of rho with data-generating value added
...
#sigma2=1
#X=matrix(c(rep(1,n),rnorm(n*3)),n,4)
#y=c((solve(diag(n)-rho*W))%*%(X%*%beta+rnorm(n))) # generating y took 36s on my PC
# run MCMC sampler
#out=MCMC.F(R=1e4,y,X,W) 
# running the sampler for 10000 iterations took 550s in total on my PC
# computing the eigenvalues of W took 276s  
# computing the ML estimates via the optim() function (MLE are used as starting values) took 184s; replacing optim() by a self-written routine would most probably speed up the procedure  

#####
# trace plot of posterior draws for rho with data-generating value added
#plot(out$rho.draws,type="l")
#abline(h=rho,col=2)
# empirical autocorrelation function for posterior draws for rho
#acf(out$rho.draws)
# marginal posterior density of rho with data-generating value added
#plot(density(out$rho.draws),xlab=expression(rho))
#abline(v=rho,col=2)
# marginal posterior density of first regression coefficient (not the intercept) with data-generating value added
#plot(density(out$beta.draws[,2]),xlab="beta1")
#abline(v=beta[2],col=2)
# marginal posterior density of sigma2 with data-generating value added
#plot(density(out$sigma2.draws),xlab=expression(sigma^2))
#abline(v=sigma2,col=2) 
